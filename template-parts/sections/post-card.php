<article class="post-<?php the_ID(); ?> rounded-[8px] overflow-hidden">
    <div class="relative mb-[15px]">
        <?php hs_first_agency_archive_post_thumbnail(); ?>
        <div class="<?php echo esc_attr(has_post_thumbnail() ? "absolute" : "relative") ?> z-[1] left-0 bottom-0">
            <?php hs_first_agency_archive_post_category(); ?>
        </div>
    </div>
    <?php hs_first_agency_archive_post_title(); ?>
    <?php
    $display_read_more_text = get_theme_mod('hs_first_agency_archive_read_more_text', __('Read More', 'hs-first-agency'));
    ?>
    <?php hs_first_agency_archive_content(); ?>
